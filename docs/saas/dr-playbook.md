# GSCRM — Disaster Recovery Playbook
...
BACKUP_OFFSITE_PATH=user@backup-host:/backups/gscrm ./scripts/backup-db.sh
```
...
1. Uygulamayı durdur: `docker compose stop gscrm-app`
2. DB volume yedekten geri yükle (bkz. `docs/deploy-vps.md` §9)
3. Flyway migrate doğrula: `docker compose run --rm gscrm-app flyway info`
...
- Wildcard nginx config yedekte: `/etc/nginx/sites-available/gscrm-multitenant`
